...
const SKILL_TREES = {
  ...
  sword: {
    color: '#b77dff',
    progression: { attacks: 'replace', upgrades: 'current' },
    forms: {
      base: {
        id: 'base', name: '相位刃', icon: '⚔️', color: '#b77dff',
        desc: '锁定近敌后以前摇提示方向，沿弧线完成近战切割',
        fire: { damage: 12, interval: 1.5, arc: 140 * Math.PI / 180, radius: 85, sweepTime: 0.22, windup: 0.14 },
        nodes: [
          { id: 'w1', name: '刃身延展', desc: '斩击半径 +28', apply(f) { f.radius += 28; } },
          { id: 'w2', name: '广角相位', desc: '斩击弧度 +50°', apply(f) { f.arc = Math.min(TAU, f.arc + 50 * Math.PI / 180); } },
          { id: 'w3', name: '高频驱动', desc: '攻击间隔缩短 25%，挥刀速度加快', apply(f) { f.interval *= 0.75; f.sweepTime *= 0.82; } },
        ],
        capstone: { id: 'w0', name: '万刃圆斩', desc: '斩击扩展为 360°，伤害 ×1.35，并产生第二道余光', apply(f) { f.arc = TAU; f.damage *= 1.35; f.echo = true; } },
        evolutions: [],
      },
    },
  },
